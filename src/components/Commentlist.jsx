...
const currentUrl = new URL(window.location.href);
    ...
    useEffect(() => { //get and post comments
  ...
  if (posting) {
    postComment(review_id, comment)
      .then((res) => {
        ...
        setComments((prev) => {
          const nextId =
            allComments?.[allComments.length - 1]?.comment_id + 1 || 1;
          return [...prev, { ...comment, comment_id: nextId }];
        });
      })
      .catch((err) => {
        console.error(err);
  ...
  setComments((prevComments) =>
    prevComments.map((comment) =>
      ...
      const userComments = comments.map((item, index, array) => {
        return <Comment
          key={item.comment_id}
          author={item.author}
...
.then((res)=> {
  ...
  getComments(review_id)
  .then((data)=> {
    setComments(data && Array.isArray(data.comment) ? data.comment : [])
  })
  setDeleting(false)
})
...
else if (err) return <h2>{err.message || String(err)}</h2>
